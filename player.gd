...
func ejecutar_ultimate() -> void:
	...
	if sfx_ulti_player:
		if sfx_ulti_player.stream == null and sfx_grito_diabolico != null:
			sfx_ulti_player.stream = sfx_grito_diabolico
		sfx_ulti_player.play()

	# --- CONFIGURACIÓN DEL ZOOM CINEMÁTICO ---
	var camara = get_viewport().get_camera_3d()
	var fov_original: float = 75.0
	if camara:
		fov_original = camara.fov
		# Zoom progresivo durante la cámara lenta
		var tween_zoom = create_tween()
		tween_zoom.tween_property(camara, "fov", fov_original * 0.4, tiempo_congelacion_ultimate).set_trans(Tween.TRANS_CUBIC)

	# --- EFECTO DE CÁMARA LENTA INTEGRAL ---
	Engine.time_scale = 0.05
	var tween_tiempo = create_tween()
	tween_tiempo.tween_property(Engine, "time_scale", 1.0, tiempo_congelacion_ultimate).set_trans(Tween.TRANS_CUBIC)

	# --- DASHEO HACIA ENEMIGO RANDOM ---
	...
	if enemigo:
		...
		tween_dash.tween_property(self, "global_position", enemigo.global_position + (dir * 0.8), tiempo_congelacion_ultimate * 0.8)
	...
	# --- RESET ZOOM ---
	if camara:
		create_tween().tween_property(camara, "fov", fov_original, 0.5)
	...
	var cd_final_ult = cooldown_ultimate * reduccion_cooldown
	await get_tree().create_timer(cd_final_ult, false).timeout
